...
const InternalNotesModal = ({ isOpen, onClose, enquiry, onNotesUpdated, showToast }) => {
  ...
  const fetchNotes = useCallback(async () => {
    if (enquiry?.id) {
      setLoading(true);
      try {
        const response = await getInternalNotes(enquiry.id, { page: 1, limit: 50 });
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      const response = await addInternalNote(enquiry.id, noteData);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await updateInternalNote(editingNote, noteData);
      setNotes((prev) =>
  ...
  const confirmDeleteNote = async () => {
    ...
    try {
      setDeleting(true);
      await deleteInternalNote(noteToDelete.id);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={`Internal Notes - Enquiry ${enquiry.id}`} size="lg">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        {/* Action Buttons */}
        <div className="flex pt-6 space-x-3 border-t border-gray-200">
          <button
            onClick={handleClose}
            disabled={submitting}
            className="flex-1 py-2.5 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 disabled:opacity-50 transition-all font-semibold active:scale-[0.98]"
          >
            Close Window
          </button>
        </div>
